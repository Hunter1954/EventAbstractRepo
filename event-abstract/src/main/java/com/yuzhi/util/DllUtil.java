...
public class DllUtil {
	...
	//加载预处理动态库
	public static Preprocessor getPreprocessor(String hownetPath){
		//加载本地dll接口
		if (preprocessor==null) {//如果变量不为空，说明已经加载，则直接返回
			System.setProperty("jna.encoding","UTF-8");
			//com.sun.jna.Native类有一个loadLibrary()方法，通过反射的方式来加载动态库
			preprocessor = Native.load(hownetPath+"TextPreprocessor",
					Preprocessor.class);
//			preprocessor = (Preprocessor) Native.loadLibrary(hownetPath+"TextPreprocessor",
//					Preprocessor.class);
		}
		//返回实例
		return preprocessor;
	...
	public static EventAbstract getDllKernel(String hownetPath) {
		//加载本地dll接口
		if (dllKernel==null) {//如果变量不为空，说明已经加载，则直接返回
			System.setProperty("jna.encoding","UTF-8");
			//com.sun.jna.Native类有一个loadLibrary()方法，通过反射的方式来加载动态库
			dllKernel = (EventAbstract) Native.load(hownetPath+"HowNet_ChineseParser_Helper",
				EventAbstract.class);
		}
		//返回实例
		return dllKernel;
