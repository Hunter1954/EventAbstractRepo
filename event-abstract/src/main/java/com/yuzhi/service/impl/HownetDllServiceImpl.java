...
@Service
public class HownetDllServiceImpl implements HownetDllService, CommandLineRunner {
...
//	 */
//    static {
//        System.out.println(hownetPath);
//        //删除记录Json解析出错句子的文件
//        DeleteFileUtil.delete(hownetPath + "sentenceFailToConvertToJson.txt");
//        //删除记录没有触发词句子的文件
//        DeleteFileUtil.delete(hownetPath + "sentenceWithoutTriger.txt");
//        //获取调用本地dll解析器的api，并初始化
//        eventAbstract = DllUtil.getDllKernel(hownetPath);
//        preprocessor = DllUtil.getPreprocessor(hownetPath);
//        System.out.println("预处理动态库加载>>>>>>" + preprocessor);
//        System.out.println("抽取动态库加载>>>>>>" + eventAbstract);
//        boolean init = eventAbstract.initHowNet(hownetPath);
//        System.out.println("抽取动态库初始化结果：" + init);
//        //读取触发词映射关系表字符流
//        try {
//            relationMappingTableReader = new BufferedReader(new InputStreamReader(new FileInputStream(new File(hownetPath + "RelationMappingTable.txt")), "UTF-8"));
...
//        System.out.println("开始前打印："+article);
//        String format=article;
        String format = preprocessor.format(article);
//        System.out.println("预处理结果："+format);
...
//						System.out.println("遇到非金融事件主体标签："+splitKey);
    ...
    private void writeIntoFileResultToJsonFailedSentence(String reason, String originSentence, String analyzeResult) {
        ...
        try {
            bw = new BufferedWriter(new OutputStreamWriter(new FileOutputStream(new File(hownetPath + "sentenceFailToConvertToJson.txt"), true), "utf-8"));
//			bw.append("\r\n");
//			bw.append(">>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>");
//			bw.append("\r\n");
//			bw.append(reason);
...
//			bw.append(">>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>");
    ...
    public void writeIntoFileSentenceWithoutMentionWord(String reason, String originSentence, List<KeenageDataDTO> nodes, ReturnJsonDTO returnJsonDTO) {
        BufferedWriter bw = null;
        try {
            bw = new BufferedWriter(new OutputStreamWriter(new FileOutputStream(new File(hownetPath + "sentenceWithoutTriger.txt"), true), "utf-8"));
            bw.append("\r\n");
            bw.append(">>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>");
            bw.append("\r\n");
            bw.append(reason);
